# dcsource.v2/django_project/sms_api.py
...
# Константы для настройки библиотеки (по умолчанию)
SMSC_LOGIN = ""                 # логин клиента
SMSC_PASSWORD = ""              # пароль клиента или API ключ
SMSC_POST = False               # использовать метод POST
SMSC_HTTPS = False              # использовать HTTPS протокол
SMSC_CHARSET = "utf-8"          # кодировка сообщения
...
class SMSC(object):
    ...
    # Конструктор теперь принимает параметры явно
    def __init__(self, login=None, password=None, debug=None):
        self.login = login if login is not None else SMSC_LOGIN
        self.password = password if password is not None else SMSC_PASSWORD
        self.debug = debug if debug is not None else SMSC_DEBUG
    ...
    def _smsc_send_cmd(self, cmd, arg=""):
        ...
        # Логика выбора login + psw или apikey (API ключ)
        if self.login:
            arg = "login=" + quote(self.login) + "&psw=" + quote(self.password) + "&fmt=1&charset=" + SMSC_CHARSET + "&" + arg
        else:
            arg = "apikey=" + quote(self.password) + "&fmt=1&charset=" + SMSC_CHARSET + "&" + arg

        # >>> ДОБАВЛЕНО: Печать полного запроса
        if self.debug:
            full_url = url + "?" + arg
            print("DEBUG SMSC FULL REQUEST:", full_url)
        # <<<
...
# Examples:
# smsc = SMSC(login="ваш_логин", password="ваш_пароль", debug=True)
